...
ames_raw$`Garage Cars` <- scale(ames_raw$`Garage Cars`)
ames_raw$`TotRms AbvGrd` <- scale(ames_raw$`TotRms AbvGrd`)
...
lmfit_2008<- lm(SalePrice ~ `Overall Qual` + Foundation + `Year Remod/Add` + (`TotRms AbvGrd`*`Lot Area`) + (`Garage Cars`*`Lot Area`), data = ames_raw_2008)
...
~~~
One type of feature engineering I applied to the model was indexing the categorical variables. For the variable `TotRms AbvGrd`, there are three levels that contain only one occurrence. There is only one house that has 2 total rooms above grade and there is only one house that has 14 rooms above grade and similar with 15 rooms above grade. Therefore, I will group all total rooms levels that have less than 10 houses under "OTHER"
